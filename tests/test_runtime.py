...
from qa_portal.runtime import CURRENT_RUNTIME_SIGNATURE, browser_url, choose_endpoint, healthcheck, load_endpoint_state, save_endpoint_state
...
class RuntimeTests(unittest.TestCase):
    ...
    def test_choose_endpoint_for_all_interfaces_uses_local_browser_url(self):
        preferred = _free_port()
        result = choose_endpoint("0.0.0.0", str(preferred), range_start=preferred, range_end=preferred)

        self.assertEqual(result["host"], "0.0.0.0")
        self.assertEqual(result["port"], preferred)
        self.assertEqual(result["url"], f"http://127.0.0.1:{preferred}")

    def test_choose_endpoint_prefers_running_scanforge_over_free_preferred_port(self):
        preferred = _free_port()
        running = _free_port()
        range_start = min(preferred, running)
        range_end = max(preferred, running)
        server = ThreadingHTTPServer(("127.0.0.1", running), _HealthHandler)
        ...
        self.assertEqual(result["port"], running)
        self.assertEqual(result["status"], "fallback-running-scanforge")
    ...
    def test_choose_endpoint_falls_back_when_preferred_port_is_foreign(self):
        ...
        server = ThreadingHTTPServer(("127.0.0.1", preferred), _ForeignHandler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        self.addCleanup(server.server_close)
        self.addCleanup(server.shutdown)
    ...
    def test_endpoint_state_for_all_interfaces_stores_local_browser_url(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            state_path = Path(temp_dir) / "endpoint.env"
            save_endpoint_state(state_path, "0.0.0.0", 8010)
            payload = load_endpoint_state(state_path)
        ...
        self.assertEqual(payload["port"], 8010)
        self.assertEqual(payload["url"], "http://127.0.0.1:8010")
        self.assertEqual(browser_url("0.0.0.0", 8010), "http://127.0.0.1:8010")
